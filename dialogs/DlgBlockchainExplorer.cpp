#include "DlgBlockchainExplorer.h"

#include <QDateTime>
#include <QKeyEvent>

inline QString utostr(unsigned int n)
{
	QString res;
	return res.sprintf("%u", n);
}

static QString makeHRef(const QString & Str)
{
	return QString("<a href=\"") + Str + "\">" + Str + "</a>";
}
/*
...
static QString ValueToString(CAmount nValue, bool AllowNegative = false)
{
...
static QString ScriptToString(const CScript& Script, bool Long = false, bool Highlight = false)
{
	if (Script.empty())
		return "unknown";
...
*/
static QString TimeToString(uint64_t Time)
{
	QDateTime timestamp;
...
static QString makeHTMLTableRow(const QStringList & lstCells)
{
	QString Result = "<tr>";
...
static const QString g_strTable = "<table>";

static QString makeHTMLTable(const QList<QStringList> & lstCells)
{
	QString Table = g_strTable;
	for (int i = 0; i < lstCells.count(); i++)
		Table += makeHTMLTableRow(lstCells[i]);
	Table += "</table>";
	return Table;
}
/*
...
DlgBlockchainExplorer::DlgBlockchainExplorer(QWidget *parent)
	: QDialog(parent)
	, m_NeverShown(true)
	, m_HistoryIndex(0)
{
	...
	connect(ui.m_btnSearch, SIGNAL(released()), this, SLOT(onSearch()));
	connect(ui.m_lContent, SIGNAL(linkActivated(const QString&)), this, SLOT(goTo(const QString&)));
	connect(ui.m_btnBack, SIGNAL(released()), this, SLOT(back()));
	connect(ui.m_btnForword, SIGNAL(released()), this, SLOT(forward()));
}

DlgBlockchainExplorer::~DlgBlockchainExplorer()
{

}

void DlgBlockchainExplorer::keyPressEvent(QKeyEvent* event)
{
...
void DlgBlockchainExplorer::showEvent(QShowEvent*)
{
/*	if (m_NeverShown)
	{
		m_NeverShown = false;
...
bool DlgBlockchainExplorer::switchTo(const QString& query)
{
/*	bool IsOk;
...
void DlgBlockchainExplorer::goTo(const QString& query)
{
	if (switchTo(query)) 
	{
		ui.m_leSearch->setText(query);
...
void DlgBlockchainExplorer::onSearch()
{
	goTo(ui.m_btnSearch->text());
}
/*
void DlgBlockchainExplorer::setBlock(CBlockIndex* pBlock)
{
	setContent(BlockToString(pBlock));
}
*/
void DlgBlockchainExplorer::setContent(const std::string& Content)
{
	...
	ui.m_lContent->setText(FullContent);
}

void DlgBlockchainExplorer::back()
{
	int NewIndex = m_HistoryIndex - 1;
	if (0 <= NewIndex && NewIndex < m_History.size())
	{
		m_HistoryIndex = NewIndex;
		ui.m_leSearch->setText(m_History[NewIndex]);
		switchTo(m_History[NewIndex]);
		updateNavButtons();
	}
}

void DlgBlockchainExplorer::forward()
{
	int NewIndex = m_HistoryIndex + 1;
	if (0 <= NewIndex && NewIndex < m_History.size()) 
	{
...
void DlgBlockchainExplorer::updateNavButtons()
{
	ui.m_btnBack->setEnabled(m_HistoryIndex - 1 >= 0);
	ui.m_btnForword->setEnabled(m_HistoryIndex + 1 < m_History.size());
}
